...
public interface MemberMissionRepository extends JpaRepository<MemberMission, MemberMissionId> {
    // 진행중, 완료한 미션 목록
    @Query("SELECT new com.example.umc9th.domain.mission.dto.MemberMissionDto(" +
            "mm.id, m.rewardPoint, s.storeName, m.missionName, mm.isCompleted) " +
            "FROM MemberMission mm " +
            "JOIN mm.mission m " +
            "JOIN m.store s " +
            "WHERE mm.member.id = :memberId")
    Page<MemberMissionDto> findByMemberId(@Param("memberId") Long memberId, Pageable pageable);
}
