...
public class ReviewController {
    ...
    ApiResponse<Page<ReviewSummaryDto>> getMyReviews(@PathVariable Long memberId,
                                        @RequestParam String storeName,
                                        @RequestParam Float rating,
                                        @PageableDefault(size = 10, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable) {

        Page<ReviewSummaryDto> page = reviewQueryServiceImpl.findMyReviews(memberId, storeName, rating, pageable);
