...
CREATE TABLE mission_assignment (
  ...
  INDEX (mission_id),
  CHECK (status IN ('ASSIGNED','IN_PROGRESS','COMPLETED')) -- 나중에 enum으로 구현
); 
...
CREATE TABLE review (
   id BIGINT PRIMARY KEY AUTO_INCREMENT,
    ...
    created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    updated_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
    FOREIGN KEY(assignment_id) REFERENCES mission_assignment(id) ON DELETE CASCADE,
...
CREATE TABLE inquiry_answer (
   inquiry_id BIGINT PRIMARY KEY,
    responder_name VARCHAR(20) NOT NULL,
...
-- notification 타입에 대한 메타 데이터 저장
-- 참조무결성 보장 (다른 데서 fk로 참조하게 하고 일관성 유지)
-- 수정에 용이함!! (ddl 날릴 필요 없이 그냥 insert 하면 됨)
...
CREATE TABLE notification (
   id BIGINT PRIMARY KEY AUTO_INCREMENT,
    member_id BIGINT NOT NULL,
    type_code VARCHAR(50) NOT NULL, -- COMPLETE MISSION.. 같은 알림의 종류를 뜻함
    title VARCHAR(120) NOT NULL,
    content TEXT NOT NULL,
    created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    ...
    FOREIGN KEY(member_id) REFERENCES member(id) ON DELETE CASCADE,
    INDEX(member_id, created_at),
    INDEX(member_id, checked_at),
    FOREIGN KEY(type_code) REFERENCES notification_type(type_code) ON DELETE CASCADE
    -- type 강제하기!!!!
);

--  notification 테이블을 조인 방식으로 상속
CREATE TABLE notification_mission_completed(
   id BIGINT PRIMARY KEY,
...
CREATE TABLE member_notification_setting(
   member_id BIGINT NOT NULL,
    ...
    email BOOLEAN NULL,
    sms BOOLEAN NULL,
    PRIMARY KEY(member_id, type_code),
    FOREIGN KEY (member_id) REFERENCES member(id) ON DELETE CASCADE,
    FOREIGN KEY (type_code) REFERENCES notification_type(type_code) ON DELETE CASCADE
);
