...
public class ReviewRepositoryImpl implements ReviewQueryDsl {
    ...
    @Override
    public Page<ReviewSummaryDto> findMyReviews(ReviewCond cond, Pageable pageable){
        ...
        // 넘길 내용들
        List<ReviewSummaryDto> content = qf
                // DTO 프로젝션으로 일부 컬럼만 DB 조회 + API 명확하게
                .select(Projections.constructor(ReviewSummaryDto.class, r.title, r.content, r.rating, r.createdAt))
                .from(r)
